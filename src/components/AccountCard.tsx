...
interface AccountCardProps {
  address: string;
  onRemove: () => void;
  onNetWorthUpdate?: (address: string, netWorth: number) => void;
}
...
export const AccountCard = forwardRef<AccountCardRef, AccountCardProps>(
  ({ address, onRemove, onNetWorthUpdate }, ref) => {
    const [selectedChainId, setSelectedChainId] = useState<string | null>(null);
    const { totalNetWorth, chainBreakdown, isLoading, isFetched, isError, refetchAll } = useMultiChainTokens(address);

    // Notify parent when net worth updates
    useEffect(() => {
      if (isFetched && onNetWorthUpdate) {
        onNetWorthUpdate(address, totalNetWorth);
      }
    }, [address, totalNetWorth, isFetched, onNetWorthUpdate]);
    ...
    return (
      <div className="bg-gray-800 rounded-lg border border-gray-700 overflow-hidden">
        ...
        <div className="p-3 border-b border-gray-700">
          ...
          {isFetched && !isError && chainsWithValue.length > 0 && (
          ...
          {isFetched && !isError && chainsWithValue.length === 0 && !isLoading && (
